  ...
  static handleResponse(http.Response response) async {
    try {
      if (response.statusCode >= 200 && response.statusCode <= 210) {
        ...
        debugPrint("statusCode: 401 Unauthorized");

        // Navigate to the login page
        // If you're using GetX for navigation
        Get.offAllNamed(
            '/login'); // Replace '/login' with the actual route name

        // If you're using Flutter's Navigator:
        // Navigator.pushReplacementNamed(context, '/login'); // You may need to pass context
