...
class _SalesScreenState extends State<SalesScreen> {
  ...
  void initState() {
    super.initState();
    allDealController.fetchMyProfile(); // Default load all deals
  }
  ...
  List<dynamic> _filterClients() {
    if (isAllDeals.value) {
      final activeList =
          allDealController.myAllClientData.value.data?.data ?? <AllDealDatum>[];
      return _applyFilters<AllDealDatum>(activeList);
    } else {
      final activeList =
          myDealController.myAllClientData.value.data?.data ?? <Datum>[];
      return _applyFilters<Datum>(activeList);
    }
  }

  // Apply filters based on search query, status, and date
  List<T> _applyFilters<T>(List<T> clients) {
    var filtered = clients;

    // Remove clients where "closer" is null
    filtered = filtered.where((client) {
      if (client is AllDealDatum) {
        return client.closer != null; // Exclude clients with closer = null
      } else if (client is Datum) {
        return client.closer != null; // Exclude clients with closer = null
      }
      return false;
    }).toList();
    ...
    if (_selectedStatus != 'All') {
      filtered = filtered.where((client) {
        if (client is AllDealDatum) {
          // Only show the clients with the "CLOSED" status if `closer` is not null
          return (client.closer?.status?.toLowerCase() ==
    ...
    if (_selectedDate != null) {
      filtered = filtered.where((client) {
        DateTime? dealDate;
        if (client is AllDealDatum) {
          dealDate = client.closer?.dealDate;
        } else if (client is Datum) {
          dealDate = client.closer?.dealDate;
        }
                ...
                Expanded(
                  child: Obx(() => CustomButton(
                    ...
                    onTap: () {
                      ...
                      allDealController.fetchMyProfile();
                    },
                  )),
                ),
                SizedBox(width: 12),
                Expanded(
                  child: Obx(() => CustomButton(
                    ...
                    onTap: () {
                      ...
                      myDealController.fetchMyProfile();
                    },
                  )),
                ),
              ],
            ...
            Obx(() {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  String tagLabel = "NEW";
                  String companyName = "";
                  String revenueTarget = "0";
                  String revenueClosed = "0";
                  String commissionEarned = "0%";
                  DateTime? startDate;
                  DateTime? endDate;
                  String clientId = "";

                  if (client is AllDealDatum) {
                    tagLabel = (client.closer?.status ?? "New").toUpperCase();
                    companyName = client.name ?? 'N/A';
                    revenueTarget =
                        _formatCurrencyDropDecimals(client.revenueTarget);
                    revenueClosed =
                        _formatCurrencyDropDecimals(client.closer?.amount);
                  ...
                  return RecentDetails(
                    ...
                    tagLabel: tagLabel.toUpperCase(),
                    companyName: companyName,
                    startDate: DateUtil.formatTimeAgo(startDate),
                    endDate: DateUtil.formatTimeAgo(endDate),
...
//   }
//
//   List<home.MyClientDatum> _filterClients() {
