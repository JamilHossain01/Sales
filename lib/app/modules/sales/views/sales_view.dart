// File: lib/app/modules/home/views/sales_content.dart (বা যেখানে আছে)
...
class _SalesContentState extends State<SalesContent> {
  ...
  List<dynamic> _filterClients() {
    var clients = dealController.myAllClientData.value.data?.data ?? [];

    if (_searchQuery.isNotEmpty) {
      clients = clients.where((c) => c.name?.toLowerCase().contains(_searchQuery.toLowerCase()) ?? false).toList();
    }

    if (_selectedStatus != 'All') {
      clients = clients.where((client) {
        final hasCloser = client.closer.isNotEmpty;
        final status = hasCloser ? (client.closer.last.status?.toLowerCase() ?? 'new') : 'new';
        return _selectedStatus == 'New' ? !hasCloser : status == _selectedStatus.toLowerCase();
      }).toList();
    }
  ...
  Color _getStatusColor(String status) {
    switch (status.toUpperCase()) {
      case 'NEW': return const Color(0xFF16A34A);
      case 'OPEN': return const Color(0xFF0094B5);
      case 'CLOSED': return const Color(0xFFE12728);
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: Container(
                    ...
                    decoration: BoxDecoration(border: Border.all(color: Colors.orange), borderRadius: BorderRadius.circular(8.r)),
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: TextField(
                            ...
                            decoration: const InputDecoration(hintText: 'Search by name..', hintStyle: TextStyle(color: Colors.orange), border: InputBorder.none),
                          ),
                        ),
                      ],
                    ),
            ...
            filteredClients.isEmpty
                ? Center(child: NoDataWidget(text: _searchQuery.isNotEmpty || _selectedDate != null || _selectedStatus != 'All' ? 'No matching results' : 'No data available'))
                : Column(
              children: filteredClients.map((client) {
                final hasCloser = client.closer.isNotEmpty;
                final String status = hasCloser ? (client.closer.last.status ?? 'New') : 'New';
                final String tagLabel = status.toUpperCase();
                final Color tagColor = _getStatusColor(tagLabel);

                final String assignDate = client.createdAt != null
                    ? DateFormat('yyyy-MM-dd hh:mm a').format(client.createdAt!)
                    : 'N/A';

                // এখানে ম্যাজিক: My Deals এ CLOSED ও OPEN এ বাটন দেখাবে, NEW তে না
                final bool showViewButton = tagLabel == 'OPEN' || tagLabel == 'CLOSED';
