...
class AllPrizeWinnersController extends GetxController {
  var isLoading = false.obs;
  var userPrizeWinnerList  = UserPrizeWinnerModel(data: []).obs;
  ...
  Future<void> fetchPrizeWinners() async {
    try {
      isLoading(true);
      final response = await BaseClient.getRequest(api: ApiUrl.userPrizeWinner);

      if (response.statusCode == 200) {
        final data = await BaseClient.handleResponse(response);
        if (data != null) {
          userPrizeWinnerList .value = UserPrizeWinnerModel.fromJson(data);
        } else {
